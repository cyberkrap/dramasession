(() => {
	...
	const PROVIDER_MARKS = {
		...
		epicgames: '<span class="connection-epic-mark" aria-hidden="true"><b>EPIC</b><small>GAMES</small></span>',
	...
	function isProfilePage() {
		return Boolean(
			...
			document.querySelector('.profile-primary-column') &&
			usernameFromPage()
		);
	}
	...
	function activityCard(activity) {
		...
		art.addEventListener('error', () => {
			art.src = '/i/Obsession/official-logo.png';
		}, {once: true});
		card.append(art);
	...
	function renderConnections(payload, username) {
		...
		if (!panel) {
			...
			const about = column.querySelector('.profile-about');
			if (about) about.insertAdjacentElement('afterend', panel);
			else column.prepend(panel);
		}
	...
	function initialize(force = false) {
		...
		if (force || changedUser || !panel || panel.dataset.connectionsUser !== username) {
			refreshConnections();
		}
	...
	document.addEventListener('visibilitychange', () => {
		if (!document.hidden) scheduleInitialize(0, true);
	});
