(() => {
	...
	function renderLoading(username) {
		const panel = panelShell(username);
	...
	function renderConnections(payload, username) {
		...
		if (!data.length) {
			...
			mount.dataset.connectionsLoadedFor = username;
			return true;
		}

		panel = panelShell(username);
		...
		if (payload.can_manage) {
			...
			edit.className = 'profile-connections-manage';
			edit.href = '/settings/connections';
			edit.innerHTML = '<i class="fas fa-cog" aria-hidden="true"></i><span>Manage</span>';
			heading.append(edit);
		}
	...
	function scheduleRefresh(delay) {
		...
		refreshTimer = window.setTimeout(() => refreshConnections(false), delay);
	}
	...
	async function refreshConnections(force = false) {
		...
		if (!username || !mount) {
			...
			retryTimer = window.setTimeout(() => queueInitialize(true), 120);
			return;
		}
		if (!force && mount.dataset.connectionsLoadedFor === username && activeRequest) return;
		...
		try {
			...
			if (version === requestVersion) {
				window.clearTimeout(retryTimer);
				retryTimer = window.setTimeout(() => refreshConnections(true), 2000);
			}
		} finally {
	...
	function initialize(force = false) {
		...
		if (!username || !mount) {
			window.clearTimeout(retryTimer);
			retryTimer = window.setTimeout(() => queueInitialize(true), 100);
			return;
		}
		const changedUser = username !== profileUsername;
		if (changedUser) {
			profileUsername = username;
			mount.dataset.connectionsLoadedFor = '';
		}
		if (force || changedUser || mount.dataset.connectionsLoadedFor !== username) refreshConnections(true);
		if (!progressTimer) progressTimer = window.setInterval(updateProgressBars, 1000);
	}

	function queueInitialize(force = false) {
	...
	const observer = new MutationObserver(() => queueInitialize(false));
	observer.observe(document.documentElement, {childList: true, subtree: true});
	document.addEventListener('visibilitychange', () => { if (!document.hidden) queueInitialize(true); });
	window.addEventListener('pageshow', () => queueInitialize(true));
	window.addEventListener('popstate', () => queueInitialize(true));
	window.addEventListener('obsession:navigation', () => queueInitialize(true));

	if (document.readyState === 'loading') {
		document.addEventListener('DOMContentLoaded', () => queueInitialize(true), {once: true});
	} else {
