(() => {
	...
	function appendHeading(panel, canManage) {
	...
	function showInitialLoading(username) {
		if (document.getElementById('profile-connections-panel')) return;
		const panel = getPanel(username);
		if (!panel) return;
		panel.replaceChildren();
		appendHeading(panel, false);
	...
	function render(payload, username) {
		...
		panel = getPanel(username);
		if (!panel) return false;
		panel.replaceChildren();
		appendHeading(panel, Boolean(payload.can_manage));
	...
	function scheduleRefresh(seconds) {
		...
		refreshTimer = window.setTimeout(() => loadConnections(true), Math.max(20, Number(seconds || 40)) * 1000);
	}
