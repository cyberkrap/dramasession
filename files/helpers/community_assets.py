...
def list_approved_assets(kind):
	"""Return approved files that actually exist, including legacy files without metadata."""
	ensure_asset_directories(kind)
	...
	return sorted(
		items,
		key=lambda item: (int(item.get("submitted_utc") or 0), item.get("id", "")),
		reverse=True,
	)
...
def approve_submission(kind, submission_id, reviewer):
	...
	item.update({"status": "approved", "reviewed_by": reviewer, "reviewed_utc": int(time.time()), "approved_url": approved_asset_url(kind, f"{item['id']}.webp")})
...
def remove_approved_asset(kind, submission_id, reviewer):
	"""Remove an approved asset while retaining submission metadata for audit/history."""
	ensure_asset_directories(kind)
	...
	for filename in os.listdir(approved_directory(kind)):
		...
		if os.path.isfile(full_path) and os.path.splitext(filename)[0] == submission_id:
			os.remove(full_path)
			removed_file = True
	if not item and not removed_file:
		raise FileNotFoundError
	if item:
