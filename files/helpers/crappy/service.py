...
def _build_comment_provider_request(
    ...
    system_prompt = (
        ...
        "\n\nTOC has its own native emotes written as :name:. Never use generic Unicode emoji in your replies. "
        "Normal text emoticons such as <3, </3, :), :(, ;), :P, :D, and similar are allowed when they fit the "
        "tone, including restrained use in otherwise professional replies. If a TOC emote would genuinely improve "
        "the tone, use search_toc_emotes with mood/action/reaction keywords and choose only a token that tool "
        "actually returned. Do not invent emote names. Most replies should use zero or one TOC emote; two is an "
        "occasional maximum, not a target. Text emoticons should also stay natural rather than being stuffed into "
        "every reply. "
