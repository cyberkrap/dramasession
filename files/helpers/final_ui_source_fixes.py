...
def patch_live_banner_source():
	with open(_LOCK_PATH, "w", encoding="utf-8") as lock:
		...
		elif new not in source: raise RuntimeError("Could not locate the live banner macro")
...
def patch_badge_gift_note_source():
	with open(_LOCK_PATH, "w", encoding="utf-8") as lock:
		...
		new = 'text += f"\\n\\n> {gift_message}"'
		if old in source: source = source.replace(old, new, 1)
		elif new not in source: raise RuntimeError("Could not locate the badge gift-message notification")
...
def patch_marseys_source():
	with open(_LOCK_PATH, "w", encoding="utf-8") as lock:
		...
		if old in source: source = source.replace(old, new, 1)
		elif new not in source: raise RuntimeError("Could not locate the marseys original-file listing")
...
def patch_asset_submission_directories_source():
	with open(_LOCK_PATH, "w", encoding="utf-8") as lock:
		...
		if marsey_save_fixed not in source:
			if marsey_save not in source: raise RuntimeError("Could not locate the marsey upload save block")
...
def patch_custom_emote_sources():
	with open(_LOCK_PATH, "w", encoding="utf-8") as lock:
		fcntl.flock(lock.fileno(), fcntl.LOCK_EX)
		js = _EMOJI_JS_PATH.read_text(encoding='utf-8')
		old_js = 'emojiIMGDOM.src = `/e/${encodeURIComponent(emoji.name)}.webp`;'
		new_js = 'emojiIMGDOM.src = emoji.url || `/e/${encodeURIComponent(emoji.name)}.webp`;'
		if old_js in js: js = js.replace(old_js, new_js, 1)
		elif new_js not in js: raise RuntimeError('Could not locate emoji image URL assignment')
		_atomic_write(_EMOJI_JS_PATH, js)

		source = _SANITIZE_PATH.read_text(encoding='utf-8')
		old = "\t\telif path.isfile(f'files/assets/images/emojis/{emoji}.webp'):\n\t\t\temoji_html = emoji_partial.format(old, f'/e/{emoji}.webp', attrs)\n"
		new = "\t\telif path.isfile(f'/asset_submissions/marseys/approved/{emoji}.webp'):\n\t\t\temoji_html = emoji_partial.format(old, f'/community-emote/{emoji}.webp', attrs)\n\t\telif path.isfile(f'files/assets/images/emojis/{emoji}.webp'):\n\t\t\temoji_html = emoji_partial.format(old, f'/e/{emoji}.webp', attrs)\n"
		if old in source: source = source.replace(old, new, 1)
