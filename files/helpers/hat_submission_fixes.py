...
from files.helpers.config.const import SITE
...
_HAT_APPROVED_ROOT = Path("files/assets/images/hats")
...
def _restore_if_different(original_path: Path, served_path: Path) -> bool:
	if not served_path.is_file():
		return False
	try:
		if filecmp.cmp(original_path, served_path, shallow=False):
			return False
		copyfile(original_path, served_path)
		return True
	except OSError:
		return False


def repair_hat_webp_copies() -> None:
	"""Restore pending and approved WebPs from their pristine archived uploads."""
	...
	for original_path in _HAT_ORIGINALS_ROOT.glob("*.webp"):
		...
		_restore_if_different(original_path, pending_path)

		approved_path = _HAT_APPROVED_ROOT / original_path.name
		if _restore_if_different(original_path, approved_path):
			repaired_approved.append(original_path.name)

	if repaired_approved:
		urls = []
		for filename in repaired_approved:
			urls.extend([
				f"https://{SITE}/i/hats/{filename}",
				f"https://{SITE}/assets/images/hats/{filename}",
			])
