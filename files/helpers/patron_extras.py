...
def ensure_patron_extras(db, user_id):
    """Best-effort badge and one-time notification fulfillment.

    This deliberately runs inside a nested transaction so an optional badge or
    notification failure cannot invalidate a verified PayPal payment.
    """
    user_id = int(user_id)
